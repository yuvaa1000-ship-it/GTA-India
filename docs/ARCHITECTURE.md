...
## SECONDARY LIFE integration (operation 06)

`Runtime -> SecondarySystem -> SecondaryVisual / FacialPerformance` extends the existing ownership graph. ParticleSurface and garment/hair patterns have no runtime dependency. SecondarySystem restores prior facial offsets before HUMAN, then applies new facial output and secondary particle motion after EUPHORIA. Final head/chest/pelvis matrices supply attachments. Full-fall head motion remains physically owned; eye/jaw expression is bounded separately.

At most nine selected character owners receive secondary simulation. Each owns two dynamic meshes/materials; the runtime owns one shared alpha fibre texture and borrows HUMAN weave maps. Character LOD/disposal changes release its secondary resources even when the original character was already removed. Far and offscreen citizens do not receive particle meshes. Hero quality is independent of local FPS. Secondary simulation adds no Rapier bodies, joints or save data.

Panel and guide positions integrate in world space at bounded substeps; visible vertices convert back into the character group space. Teleports reset rest state. Capsule/floor contacts are one-way projection approximations, and garment bend uses second-neighbour distance constraints. Gaze events and timestamped phoneme cues are explicit inputs, not AI understanding or audio-derived phonemes. See SECONDARY_LIFE_SPEC for exact limits and SYSTEM_STATUS for executable acceptance.
